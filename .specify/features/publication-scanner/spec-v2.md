...
**Created**: 2026-03-26  
**Updated**: 2026-04-05  
**Status**: Draft — v2 (extends v1 with email delivery, feedback loop, multi-source, CLI CRUD)  
**Spec Version**: 2.0  
**Previous Version**: `spec-v1.md` (archived, unchanged)  
...
7. Collects feedback to refine future relevance scoring
...
## Feedback System 🔲 v0.2 (NEW)

The feedback loop enables users to rate publications in the digest, improving future scoring. Two input mechanisms are provided: an HTTP callback server (for email link clicks) and a CLI command (for manual/offline use).

### Feedback HTTP Server

| Property | Value |
|---|---|
| **Host** | `localhost` |
| **Port** | `8230` (configurable via `--port`) |
| **Protocol** | HTTP (localhost only — no TLS needed) |
| **Python framework** | `http.server` (stdlib) — minimal, no external deps |
| **Lifecycle** | Start-on-demand via `pubscout feedback serve`; user stops with Ctrl+C or `--timeout` |

**Endpoints**:

| Method | Path | Parameters | Response |
|---|---|---|---|
| GET | `/feedback` | `id` (publication ID), `signal` (`positive` or `negative`) | HTML "Thanks!" page with publication title confirmation |
| GET | `/health` | — | JSON `{"status": "ok", "port": 8230}` |

**Recording**:
- On each feedback request, call `database.save_feedback(pub_id, signal, timestamp)`.
- If the publication ID is unknown, return a friendly error page: "Publication not found — it may have expired from the database."
- Log each feedback signal to console for visibility.

**Graceful degradation** (when server is not running):
- Feedback links in the email will fail with a connection-refused error.
- The email footer includes a fallback note: "Feedback server not running? Use: `pubscout feedback <id> up` or `pubscout feedback <id> down`"

> **Design decision** (resolves Open Question 1): The feedback server runs on-demand via `pubscout feedback serve`, NOT as a persistent daemon. Rationale: avoids background resource consumption; users only need it when actively reading a digest. A `--timeout 3600` option auto-stops after inactivity.

### Feedback CLI Command

```
pubscout feedback <publication-id> <up|down> [--note "Optional note"]
pubscout feedback list [--limit 20] [--signal positive|negative]
pubscout feedback serve [--port 8230] [--timeout 3600]
```

- `feedback <id> up/down` — records a feedback signal directly to SQLite
- `feedback list` — shows recent feedback entries (date, pub title, signal, note)
- `feedback serve` — starts the HTTP callback server
...
## Relevance Tuning CLI 🔲 v0.2 (NEW)

CLI commands to modify scoring configuration in `profile.yaml` without manual YAML editing.

### Commands

```
pubscout config show                          # Show current scoring config
...
✅ Added "survey" to exclude keywords. Publications containing this word will be filtered out.
```
